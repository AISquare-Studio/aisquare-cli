...
def no_real_tmux(monkeypatch: pytest.MonkeyPatch) -> Iterator[list[tuple[str, ...]]]:
    ...
    yield ran
    # `tmux -V` names no socket and touches no server: a real pane asks it once,
    # before its first forwarded key, to learn whether extended chords carry (#147).
    wrong = [
        argv for argv in ran if _socket_of(argv) != PRIVATE_SOCKET and list(argv[1:]) != ["-V"]
    ]
