...
for — is named in a quiet notice instead, once per key name in that pane: `no
way to type f13 into a tmux pane`. Nothing is sent either way, since mistyping
into a running agent is the worse failure.
