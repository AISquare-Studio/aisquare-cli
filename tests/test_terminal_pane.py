...
class Host(App[None]):
    ...
    ``on_text_selected`` mirrors ``FleetApp``: the Screen posts it on every
    MouseUp and it bubbles to the app, never to a widget, so the app is where
    a pane learns that a selection gesture finished.
    """
