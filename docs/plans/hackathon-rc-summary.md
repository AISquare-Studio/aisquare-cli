...
> `rc/hackathon-v1` → `main` pull request. One row per feature. Nineteen PRs in total; every row was
...
| **Stop an agent from `asq`** — Stop on the agent view, `x` on the sidebar | The operator spawns AND stops agents without leaving the UI; no `fleet spawn` / `fleet stop` in a second terminal. | One confirm dialog (*Stop* · *Force* · *Cancel*) over the same `services.fleet.stop` the CLI runs, `--force` included; a refusal stays in the dialog and the agent keeps running; a stop toasts the receipt, the list re-reads and the view leaves the pane that is gone. | `cli/ui/stop.py` over the unchanged service (0 lines changed there); both controls ask the same `sidebar.ALIVE_STATES`, so a control that cannot be shown cannot be reached by key; the dialog reads the grace period from the service's signature rather than typing it. | P21 #202 |
...
- **Store migration:** the two nullable columns (`team_session.persona`, `fleet_agent.persona`) are no numbered step of their own since the release train (#203) was folded in: its ladder claimed v15 too (the account registry, then v16–v21), and a store stamped 15 by either side can never be renumbered onto the other. So `_PREPARE[15] = _converge_v15_fork` (#208, on the `_converge_v11_fork` precedent) adds both sides' v15 idempotently before v16 runs, and `_converge_v15_shape` repairs at open a store the fork already carried past 15; every cohort — `main`'s v14, a persona-15 store, an accounts-15 store, and one run to 16 or 21 without the step — reaches v21 with both shapes (`tests/test_store.py`, the cohort tests). `docs/store-migration-race.md` still governs how the statements run.
