...
`AISQUARE_PERSONA` env (the launch-time value, and all a hand-typed launch with
no row has) > the session row's stored persona (P20; see §10). The CLI verb is
`aisquare persona attach <name> --to <label>`.
...
| 2026-09-15 | **P5 as built** (`feat/persona-import`, branched from P2's PR head). §5's names as written (`PersonaDraft`, `EngineUnavailable`, `draft_with_manager`, `draft_with_api`, `draft`, `ImportRefused`, `PersonaImportSettings`), with additive keywords recorded here: the engines and `draft` take `feedback` (the validator's rule for the one retry), and `draft_with_api`/`draft` take `progress` (so usage tokens reach the operator without the service printing). `import_source` gains `stdin: bytes \| None = None` — coder3b-1's seam for P6's paste box, so provenance records `stdin` rather than a temp path. `services.personas.DraftKept(PersonaError)` carries `draft_path`; every engine draft is written to `.drafts/<name>/SKILL.md` BEFORE validation outcome or confirmation, so `import_invalid`, `not_confirmed` (the CLI reports `needs_confirmation` when it could not ask) and `persona_exists` all name a path, and importing that path finishes on the recognised path with the engine's provenance carried. `--condense` also accepts a skill over the 12,000 hard cap (the remedy §3.5 names; 45 of 80 real skills measured on this machine exceed it). `[persona.import]` is `PersonaSettings.import_` with alias `import` and `serialize_by_alias`, and `config._keep_unknown` now knows fields by alias, or a newer build's key inside the section would be lost. `anthropic>=1` joins the dev extra (the `mcp` precedent) so mypy --strict checks the real `messages.parse` call; conftest's `no_real_llm_import` makes any test that reaches an engine fail over to `no_import_engine`. **Answered by the manager (both YES):** (1) `urllib.request` already loads at CLI import (services/ci_client.py, services/explainability.py), so the importtime bullet reads "P5 adds no module-level `anthropic`, `urllib.request` or `yaml`, and `persona_import` stays out of the CLI and hook import closure" — the pre-existing load is recorded in the test, not asserted away (seq 7097); (2) `--bare` reads no OAuth per Claude Code 2.1.272's help, so it is dropped and `probe_model`'s isolation kept exactly — cwd the aisquare home, `--settings {}`, `--strict-mcp-config`, `--tools ""`, `--no-session-persistence`, `--max-turns 1`, the stripped env with `AISQUARE_TEAM=0` and `CLAUDE_CODE_DISABLE_ADVISOR_TOOL=1` (seq 7100). | coder3a-1 `022c1186` |
| 2026-09-15 | **P8 as built** (`feat/persona-attach`, branched from P5's PR head). `services.fleet.attach_persona(project, label, name, *, sender=None) -> AttachReceipt(agent, persona, replaced, delivered: "typed" \| "noted", how)` as §5 names it. Order is the refusal order: resolve the persona (unknown → `FleetError` with the known names, before the store or tmux), find the live agent (`NoSuchAgent`), write the one `persona_attached` board event through `team.add_note` (which also refuses an unknown `--as` before any row changes), record the name via new store methods `set_fleet_agent_persona` and `set_session_persona` (the latter only when the agent's session row exists), then deliver the preface line plus `briefing()` through the existing `tell` — no second channel. `replaced` is the previous persona only when it differs (re-attaching the same one names no replacement). The hook's fallback is `services.team._asked_persona`: `AISQUARE_PERSONA`, else the persona on the row `_fleet_row_named` returns; P2's `COALESCE` then keeps the session row's persona when neither asks, which is exactly env > row > session. CLI `persona attach` reuses `cli.fleet`'s `ProjectRef`/`SessionRef`/`_project`/`_fail_fleet` so its error codes are the fleet's. Follow-up when #144 lands: `persona` joins `LaunchSpec` so a restarted agent replays it (not implemented here). | coder3a-1 `022c1186` |
| 2026-09-15 | **P3 as built (§4.1 details the table left open).** (1) Textual 8.2.8's `Select` has no per-option disabled state, so "manager greyed" is the overlay's own `OptionList.disable_option_at_index` plus a dim "one per project, already running" prompt, and — belt to those braces — picking it anyway shows "<project> already has a manager — one per project" and disables *Spawn*. (2) The manager role locks the Label field at `manager` (the service ignores any other); a label typed before comes back when the role changes away. (3) In a non-git project the worktree switch is off and disabled, and a role whose default IS a worktree is sent `worktree=False` — the one untouched field sent as a value, because the service would refuse its `None`. (4) "Untouched" is measured against the default, not an edit flag: a field equal to the role's default (or the label equal to its prefill) is sent as `None`; untouched Worktree/Permission mode/Binary placeholder follow a role change, touched ones stay. (5) `SpawnDialog(project, *, accounts=reader)` — the Accounts reader is injected (the app passes its own, tests a stub); P4 adds the presets beside it. (6) While a spawn runs, *Cancel* is disabled and `Esc` does nothing: a started spawn cannot be taken back, and dismissing would lose its receipt. (7) Label check also refuses `manager` for another role, as `next_label` does. (8) **Textual trap for every screen P4/P6/P7 add:** never name a private attribute `_running` on a `Screen`/`Widget` — it is `MessagePump._running`, and shadowing it leaves every button and binding on that screen silently dead (measured: *Spawn* disabled from open, `Esc` ignored, no exception). Scan a new attribute name against `textual/{message_pump,dom,widget,screen}.py` first; the dialog's flag is `_spawning`. | coder3b-1 (`1cff41aa`) |
| 2026-09-15 | **P20: the fleet row beats the variable** (`fix/persona-row-beats-env`, stacked on P8's head). Peer review #194 (seq 7590) found that an attachment did not survive `/clear` for an agent spawned with `--persona`: `launch` exports `AISQUARE_PERSONA` at spawn time, and `_asked_persona` read it before the fleet row that `attach_persona` updates. The manager ruled that `hook_session_start` resolves the persona in this order: (1) the `fleet_agent` row's persona, when that row carries one, because it is the latest recorded intent (`fleet spawn` writes `--persona` there and `attach` replaces it); (2) `AISQUARE_PERSONA`, the launch-time value and all a hand-typed launch with no row has; (3) the session row. A row without a persona does not silence the variable, and an unknown persona on the row gets the one "launched without it" line. P8's `test_the_variable_beats_the_row_and_the_row_beats_the_session` is replaced, not deleted, by `test_the_row_beats_the_variable_and_the_variable_beats_the_session`. The row-read fail-open (P19's 🔴 on #186) now logs a warning naming the fleet agent id and project id, with the traceback (`services.team._log`), and adds one briefing line, `persona: the fleet row could not be read (<ExceptionClass>) — started without an attached persona`, instead of passing silently (the amendment from peer review #186's second 🔴). §4.7 and docs/personas.md state the new order. | coder3a-1 `022c1186` |
| 2026-09-15 | **P4 as built** (`feat/hack-p4-persona-in-spawn`: `feat/persona-wiring` @ `fc8781a` with `feat/hack-p3-spawn-dialog` merged in, while #181/#183/#180 awaited merge — rebase onto `rc/hackathon-v1` once they land). (1) The dialog's field order is Role (+ *Pick…*) · Account · Binary · **Persona** (+ description) · Label · Task · Worktree · Permission mode · Extra agent args · First prompt — who runs it, then as whom. (2) `persona=` follows the dialog's own rule: `None` while the field shows the role's default (the service applies the config), the name once chosen, and `""` for an explicit *(none)* over a role that has a default — P2's `_chosen_persona` takes a non-`None` flag and reads an empty name as none, so no new seam was needed. (3) Presets count as a choice and are sent; a preset role missing from the list (a seat, `coder2`) and a preset account the accounts read has not produced are added as options. (4) The persona options are built once at compose and include every role's configured default and the preset even when the catalogue lacks them (shown as `<name> — not one of this project's personas`, the spawn refuses it naming the key): following the role then only ever sets a value, never `set_options`, which would post a Changed for `(none)` and read as the user's pick. (5) The catalogue root is `project.root` — the root `spawn` resolves a persona against — in the dialog and the Settings tab alike (P6's tab uses `git_common_root`, the CLI's root; for a git project they coincide). (6) Until P7, the dialog answers its own `PickTargetRequested` with the toast "target picker arrives with P7" and lets it bubble. (7) The sidebar badge reads `FleetAgent.persona`, else the session's `persona`; the row's wins. | coder3b-1 `1cff41aa` |
| 2026-09-15 | **P6 as built** (`feat/hack-p6-personas-tab`, branched from P1's PR head `4f7367e` while #181 was in review). (1) The tab's project layer root is `git_common_root(project.root)` — the CLI's rule — so a non-git project shows user + bundled only, and Import/New/Export disable the project layer there. (2) **Paste text…** is not in this build: `import_source('-')` reads `sys.stdin`, which a TUI cannot hand it, and a temp file would record the wrong provenance; the seam `import_source(..., stdin: bytes | None = None)` was asked for on the board — wiring it is one Switch and one kwarg. (3) `[persona.import]` is P5's; until it exists the Import dialog shows the documented defaults as named constants (`DEFAULT_ENGINE = "auto"`, `DEFAULT_API_MODEL = "claude-opus-5"`, LLM switch on) and sends `engine=None` untouched, `model=None` empty, so `import_source` decides; P5 swaps the constants for the config in one line. (4) Until P7, the tab answers its own `AttachRequested` with the toast "target picker arrives with P7" and does not stop the message, so P7 handles it above and deletes that handler. (5) Bundled rows disable Edit/Remove as §4.2 says, and `Enter` on any row opens the SKILL.md — read-only with *Save as…* for bundled (§4.4) — so both sections hold; *Save as…* is `export(to=<layer dir>)`. (6) *Browse skills* fills Source with the skill's **path**, not its name, so a same-named directory in the UI's cwd can never win the resolution. (7) New's description is applied to the editor's text (re-rendered with `core.render`), not written by `new()`; *Save* writes it through `save()` like any edit. (8) Progress lines accumulate under the form for the run (the §7 "both texts in order" is the artefact). (9) Textual traps, again, for P4/P7: `visible` (DOMNode), `_attach` (MessagePump) and `layer` (Widget — a CSS render layer: `self.layer = "user"` puts a screen on a layer that does not exist) are taken; mypy caught the first two, a scan caught the third. Scan every new attribute and method name against `textual/{widget,dom,screen,message_pump}.py`. (10) `tests/test_ui_project.py`'s tab test now expects six tabs, `tab-personas` last. | coder3b-1 `1cff41aa` |
| 2026-09-15 | **P7 as built** (`feat/hack-p7-target-picker`: `feat/persona-attach` @ `509fc8b` with `feat/hack-p4-persona-in-spawn` and `feat/hack-p6-personas-tab` merged in, conflicts resolved keeping both sides — the `docs/personas.md` intro by hand). (1) The picker answers WHO and does not act: `AttachTargetScreen` dismisses with a `Target`, and the screen that opened it acts — the Personas tab answers its own `AttachRequested` (P6's toast seam replaced), the Spawn dialog its own `PickTargetRequested` (P4's) — so there is one picker and no second attach or spawn path. (2) *Pick…* on an OPEN dialog fills Role/Binary/Account through the fields' own change handlers (a role missing from the list is added; the manager option is greyed again) — P4's compose-only rule is about opening the dialog preset, and re-opening would lose what was typed. A picked **agent** from *Pick…* fills its role and binary (spawn another like it). (3) The seat rule is `fleet_service._role_ok` — the private check `spawn` applies, over `cli.launch._role_ok`; `team bind` validates no name; a public name was asked for on the board, not added (BOUNDARIES). (4) **+ New account** is a navigation: the picker dismisses with `Target('new-account')`, its owner posts `NewAccountRequested`, the app shows Accounts and calls `AccountsView.begin_claude_sign_in(None)`; the return hop to the picker is not in v1. (5) Rows are read in a thread worker (`list_agents` asks tmux); the Accounts section is empty with a note when the opener passed no accounts reader. (6) A spawn opened from the tab reports through `SpawnCompleted`, which the app turns into its usual receipt path (toast, refresh, select). (7) Import… in the dialog uses `git_common_root(project.root)`, as the tab does. | coder3b-1 `1cff41aa` |
| 2026-09-16 | **P21 as built** (`feat/hack-p21-ui-stop-agent`, branched from `rc/hackathon-v1`). The Spawn dialog's twin: stopping an agent from the TUI. (1) Both controls ASK rather than act — the agent view's compact `#agent-stop` button (id and placement taken from PR #163 so the two converge when its Restart lands beside it) and the Sidebar's `x` both post one `StopAgent(project_id, agent_id)`, and the shell answers it, because the shell is what holds the `ProjectInfo` a stop needs and what owns dialogs (`on_spawn_agent`'s shape). (2) Stop's visibility asks `sidebar.ALIVE_STATES`, the constant the card's "agents alive" chip counts by, instead of a second list of states that could drift from it — the P13/P17 lesson; exited and lost rows therefore show no Stop, and a lost row stays `fleet reap`'s business. (3) `x` is a **Sidebar** binding acting on `selected_key`, not an AgentRow one: the rows are non-focusable Statics on purpose (§4.3, so the list does not eat the arrows), so there is no focused row to bind to; a non-agent selection does nothing. (4) The dialog owns the service call, as `SpawnDialog` owns its spawn: a refusal is an answer to show, so it lands on the status line as a `rich.text.Text` with the dialog open and the agent untouched. That matters more for `stop` than for `spawn` — `stop` refuses on purpose when tmux cannot CONFIRM the pane died, and a dialog that closed on that answer would restore the "✓ stopped" lie the service exists to prevent. (5) The grace seconds in the dialog's sentence are read from `stop`'s own signature rather than typed, so the promise cannot outlive the default. (6) The manager line keys on `fleet_service.MANAGER_LABEL`, the name `spawn` reserves. (7) **The board event in the contract's deliverable 5 does not exist to emit:** `services.fleet.stop` writes no team event on rc (its path ends at `store.end_fleet_agent`, a plain UPDATE), and adding one would mean editing `services/fleet.py`, which BOUNDARIES forbid — so the UI writes none and the count stays the service's, posted to the board rather than decided here. (8) Tests went to `tests/test_ui_shell.py`: the addendum named `tests/test_ui_project.py`, which has zero agent-view references, while the shell file already holds both the agent view and the Sidebar key tests. (9) **Peer finding, fixed before the PR opened:** `x` applied no state rule, so on a 💤 exited row it opened the very dialog the button refuses — and `stop` does not decline such a row (pane gone, so no `/exit` and no grace: it ends the row outright, which is `fleet reap`'s outcome). Both controls now ask the same `ALIVE_STATES`, with the negative test coder3a-1's probe asked for. Read as ruled by the contract rather than by me: BOUNDARIES puts reap and dead-row semantics outside P21, so a key that ends a dead row was out of scope, not a choice. | coder3b-1 `1cff41aa` |
