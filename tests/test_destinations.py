...
def test_a_fix_for_a_projects_deployment_never_moves_the_machines_target_of_that_name(
    isolated_home: Path, tmp_path: Path
) -> None:
    """On the machine ``init --explainability`` writes, the machine's own target is
    ``stg`` by default and, with no entry, reads the top-level prod gateway, proxy and key
    file; a project ``use``d on staging is ``stg`` too, and both read
    ``[explainability.targets."stg"]``. The proxy lane's fix for the project named that
    entry alone, and written as it said, every project without a destination sent its
    model traffic and the prod key through the staging proxy (final review of #203, EX1).
    The fix moves the machine's target off the name first: followed word for word, the
    machine reads what it did and the project's deployment takes the proxy. A machine
    whose own target is the same deployment gets the entry alone."""
    prod_gateway = "https://explainability-api.aisquare.studio"
    config = AppConfig()
    config.explainability.enabled = True
    config.explainability.gateway_url = prod_gateway
    config.explainability.proxy_url = f"{prod_gateway}:9443"
    save_config(config)
    service.store_api_key("AIS_machine_prod_key")
    project = _project(tmp_path / "web")
    session = iam.Session(api_url="https://stg-api.aisquare.studio", token="aisq_x", source="env")
    ...
    def machine() -> tuple[str, str, str, str | None]:
        resolved = ops.resolve_target(load_config().explainability, None)
        return (resolved.gateway_url, resolved.proxy_url, resolved.key_source, resolved.api_key)

    def proxy_fix() -> str:
        target = ops.resolve_target(load_config().explainability, None, project_id=project.id)
        state = ops.proxy_state(
            replace(target, proxy_url="https://proxy.elsewhere.example:9443"),
            on=True,
            prober=lambda _url: service.ProxyProbe(True, "healthy"),
        )
        return " ".join(state.remediation.split())

    before = machine()
    assert before == (prod_gateway, f"{prod_gateway}:9443", "file", "AIS_machine_prod_key")
    hosted = "https://stg-explainability-api.aisquare.studio:9443"
    said = proxy_fix()
    assert f'proxy_url = "{hosted}" under [explainability.targets."stg"]' in said
    assert f"it is 'stg' too, on {prod_gateway}" in said
    assert 'Set target = "<name>" under [explainability]' in said, said
    assert f"its gateway stays {prod_gateway}" in said

    # Followed word for word: the machine's target renamed, then the entry written.
    config = load_config()
    config.explainability.target = "machine"
    config.explainability.targets["stg"] = ExplainabilityTarget(proxy_url=hosted)
    save_config(config)
    assert machine() == before, "every project without a destination reads what it did"
    moved = ops.resolve_target(load_config().explainability, None, project_id=project.id)
    assert (moved.gateway_url, moved.proxy_url) == (
        "https://stg-explainability-api.aisquare.studio",
        hosted,
    )
    assert moved.machine_shares_entry is None and "Set target" not in proxy_fix()

    # A machine whose own `stg` IS staging reads the entry for the same deployment.
    config = load_config()
    config.explainability.target = "stg"
    config.explainability.targets["stg"].gateway_url = moved.gateway_url
    save_config(config)
    assert "Set target" not in proxy_fix()
