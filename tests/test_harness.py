...
def test_spawn_exec_starts_the_agent_on_the_id_it_traces_under(
    ...
    id in argv is the id in ``X-Pipeline-Id``, and the agent carries what its
    own hook needs to close the join."""
    import uuid
...
def test_spawn_exec_on_a_bound_wrapper_traces_and_still_joins(
    isolated_home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """``--bin`` names an executable we know nothing about, so nothing is added
    to its argv — and it joins anyway, because the pipeline id rides in the
    ENVIRONMENT. This is the case that decides the design."""
    from aisquare.services import hooks
    from aisquare.services.explainability import join_records
    from tests.proxy_stub import healthy_proxy
    ...
    monkeypatch.setattr("aisquare.cli.team.shutil.which", lambda _name: "/usr/bin/my-wrapper")
    monkeypatch.setattr("aisquare.cli.team.os.execvpe", _fake_exec)
    runner, app = _cli()
    with healthy_proxy() as proxy_url:
        _tracing_enabled(proxy_url)
        argv_in = ["team", "spawn", "coder", "--exec", "--bin", "my-wrapper"]
        result = runner.invoke(app, argv_in)  # type: ignore[arg-type]

    assert result.exit_code == 0, result.output
    argv = calls["argv"]
    assert isinstance(argv, list)
    assert argv[0] == "my-wrapper"
    assert "--session-id" not in argv, "never hand a flag to a binary we did not resolve"
    env = calls["env"]
    assert isinstance(env, dict)
    assert "X-Pipeline-Id" in env["ANTHROPIC_CUSTOM_HEADERS"], "still traced"

    monkeypatch.setenv("AISQUARE_PIPELINE_ID", env["AISQUARE_PIPELINE_ID"])
    monkeypatch.setenv("AISQUARE_AGENT_NAME", env["AISQUARE_AGENT_NAME"])
    assert hooks.record_trace_join("board-row-77") is None
    (record,) = join_records()
    assert record["session_id"] == "board-row-77"
    assert record["pipeline_id"] == env["AISQUARE_PIPELINE_ID"]
