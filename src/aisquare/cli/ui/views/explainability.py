...
def attach_project_key(value: str, project: ProjectInfo, target: str) -> Notice:
    ...
    deployment the form saved its settings for. It was a separate *Attach key*
...
class ExplainabilityView(VerticalScroll):
    ...
    def _save_setup(self) -> None:
        ...
        page's agents launch into, for the deployment these settings were saved
        for, instead of written to the machine file — ``key set``'s write, the
        one :func:`attach_project_key` makes.
        """
        ...
        if owner is not None:
            key_target = ops.resolve_target(settings, target or None).name
