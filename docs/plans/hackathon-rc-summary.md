...
- **Merges into rc were performed under the owner's overnight authorisation** with a merge gate of runner verdict + CI green + manager review; the framework (`/pr-review`) comments were added before the final merge pass.
- **Known follow-up:** P17 switches the picker's seat check to the public `role_ok` (one line) — landing in this branch right after #188 and #192.
