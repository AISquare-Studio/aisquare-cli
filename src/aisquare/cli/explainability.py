...
from aisquare.services.explainability import probe_proxy, trace_marker, wire_session
...
def env(
    ...
    ``AISQUARE_PIPELINE_ID`` and ``AISQUARE_AGENT_NAME`` are exported
    alongside the header pair. Any agent started from this shell inherits
    them, and the ``aisquare`` hook running inside it records the
    session→Run join — so a terminal wired by hand joins exactly like one the
    launcher managed, whatever binary it runs. A caller that knows it is about
    to run claude can also start it ON that id
    (``claude --session-id "$AISQUARE_PIPELINE_ID"``) to make the two ids
    identical rather than merely joined.
    """
    wiring = wire_session(
    ...
    exports = {**wiring.env, **trace_marker(wiring)}
