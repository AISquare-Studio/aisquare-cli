    ...
    *, live: bool = False, target: str | None = None, cwd: Path | None = None
    ...
    return [
        ...
        *_claude_accounts_checks(),
        ...
        # `xr` is an optional extra, not a machine fault, so it sits below the
        # actionable rows for the same reason `browser_tools` does: a missing
        # `[xr]` install must not evict `brain` / `snapshot` / `gh` from the
        # three rows the sidebar shows.
        _check_xr(),
        *_experiment_checks(),
